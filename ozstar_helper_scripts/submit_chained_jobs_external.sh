...
# Usage: ./submit_chained_jobs_external.sh FIELDNAME
# Example: ./submit_chained_jobs_external.sh aqr-00
...
expected_count=$(python3 -c "from astropy.table import Table; prods = Table.read('$fits_table', '$FIELD'); print(len(prods['productFilename']))")
...
if [ "$actual_uncal" -eq "$expected_count" ]; then
  ...
  python generate_slurm_script_download.py --field "$FIELD" --cpus $DL_CPUS --mem $DL_MEM --time $DL_TIME
...
if [ "$actual_rate" -eq "$expected_count" ]; then
  ...
  python generate_slurm_script_stage1.py --field "$FIELD" --cpu $STAGE1_CPUS --mem $STAGE1_MEM --time $STAGE1_TIME
...
if [ "$preprocess_done" = false ]; then
  if is_job_pending_or_running "$preproc_jobname"; then
    ...
    python generate_slurm_script_preprocess.py --field "$FIELD" --cpu $PRE_CPUS --mem $PRE_MEM --time $PRE_TIME
...
if [ -f "$phot_fits" ] && [ -f "$ircat_fits" ]; then
  ...
  if is_job_pending_or_running "$mosaic_jobname"; then
    ...
    python generate_slurm_script_mosaic.py --field "$FIELD" --cpu $MOS_CPUS --mem $MOS_MEM --time $MOS_TIME
...
if [ "$n_clean" -gt 0 ]; then
  ...
  python generate_slurm_script_contam.py --field "$FIELD" --cpu $CONTAM_CPUS --mem $CONTAM_MEM --time $CONTAM_TIME
...
if [ "$skip_downstream" = true ]; then
  echo "[⚠️] Skipping extract, zfit, biopage, and refit — no grism data available."
	...
	naxis2=$(python3 -c "from astropy.io import fits; h = fits.getheader('$extracted_fits', 1); print(h.get('NAXIS2', 0))")
	...
	if is_job_pending_or_running "$extract_jobname"; then
	    ...
	    python generate_slurm_script_extract.py --field "$FIELD" --cpu $EXTRACT_CPUS --mem $EXTRACT_MEM --time $EXTRACT_TIME
	...
	python generate_slurm_script_zfit.py --field "$FIELD" --cpu $ZFIT_CPUS --mem $ZFIT_MEM --time $ZFIT_TIME
	...
	if is_job_pending_or_running "$biopage_jobname"; then
	    ...
	    python generate_slurm_script_biopage.py --field "$FIELD" --cpu $BIOPAGE_CPUS --mem $BIOPAGE_MEM --time $BIOPAGE_TIME --root "$root" --useprior 0
	...
	if is_job_pending_or_running "$refit_jobname"; then
	    ...
	    python generate_slurm_script_refitEWprior.py --fieldname "$FIELD" --cpu $REFIT_CPUS --mem $REFIT_MEM --time $REFIT_TIME
	...
	if is_job_pending_or_running "$biopage_ew_jobname"; then
	    ...
	    python generate_slurm_script_biopage.py --field "$FIELD" --cpu $BIOPAGE_CPUS --mem $BIOPAGE_MEM --time $BIOPAGE_TIME --root "$root" --useprior 1

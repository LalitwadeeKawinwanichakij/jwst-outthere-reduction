...
#SBATCH --job-name={field}_zfit_mw         # Job name
...
#SBATCH --output=FIELDS/{field}/logs/zfit_mw_%j.out  # Standard output log
#SBATCH --error=FIELDS/{field}/logs/zfit_mw_%j.err   # Standard error log
...
echo "Running redshiftFit for {field} with {cpus_per_task} CPUs"
pixi run --no-lockfile-update --environment grizli \
  ./workflow/scripts/redshiftFit_externalphot_4ozstar.py {field} \
  --ncpu {cpus_per_task} \
  > FIELDS/{field}/logs/zfit_mw.log 2>&1
...
def main():
    ...
    with open(f"{args.field}_zfitmw.slurm", "w") as file:
        file.write(slurm_script)

    print(f"SLURM script for {args.field} created: {args.field}_zfitmw.slurm")

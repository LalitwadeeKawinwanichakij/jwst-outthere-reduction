...
#SBATCH --time={time}:00:00          # Max runtime (hours)
#SBATCH --tmp=20GB
...
if [ ! -d "$JOBFS/data/lkawin/FIELDS/{fieldname}/RedshiftFitting" ]; then
    ./create_symlink_extractions.sh {fieldname}
fi

pixi run --no-lockfile-update --environment grizli ./resources/scripts/Postprocessing_logLikeEW_4OzStar.py --field {fieldname} --output_dir $JOBFS/data/lkawin/FIELDS/{fieldname}/RedshiftFitting > FIELDS/{fieldname}/logs/zfitEW.log 2>&1
...
if __name__ == "__main__":
    ...
    parser.add_argument("--cpu", type=int, default=1, help="Number of CPUs per task (default: 1)")
    parser.add_argument("--mem", type=int, default=16, help="Memory per CPU in GB (default: 16)")
    parser.add_argument("--time", type=int, default=72, help="Maximum runtime in hours (default: 72)")

...
prep_dir= '/fred/oz408/NISPureParallel/FIELDS/%s/Prep'  % root # modify if needed
extract_dir= '/fred/oz408/NISPureParallel/FIELDS/%s/Extractions'  % root
    ...
    if len(glob.glob(full_file)) > 0:
        ...
        while attempt < max_iterations and z_best_new == -99:
            ...
            for i, z_fit in enumerate(solution_redshifts):
                ...
                new_args['mb_files']= [mb_file]
                _mb, _, fit, tfit, line_hdu, cov_hdu = fitting.run_all(object_id, **new_args)
        ...
        if z_best_new > 0 and z_best_new != full_hdul['ZFIT_BEAM'].header['Z_MAP']:
            ...
            final_args['mb_files']= [mb_file]
            fitting.run_all(object_id, **final_args)
        return z_best_new
...
def process_object(object_id):
    ...
    full_path = f"{new_redshift_dir}/{root}_{str(object_id).zfill(5)}.full.fits"
    log_path = f"{new_redshift_dir}/{root}_{str(object_id).zfill(5)}_nonewsolution.log"

    if (os.path.islink(full_path)==True) &  (not os.path.exists(log_path)):
        try:
            print(f"Processing object {object_id}")
            z_best_new = iterative_redshift_search(object_id=object_id, zspec=None, showplot=False, max_iterations=2)
        ...
        print("Skipping %d" % object_id)
        return None
def main():
    npz = np.load(f'{new_redshift_dir}/{root}_objectid_2refit.npz')
    object_id_toberefit = npz['object_id'].astype(int)
    print(f"Total objects to refit: {len(object_id_toberefit)}")
    #process_object( object_id_toberefit[0])
    # Use multiprocessing
    with mp.Pool(processes=min(8, mp.cpu_count())) as pool:
        pool.map(process_object, object_id_toberefit)

    # Consolidate results after parallel processing
    #print('Combining all results...')
    #ids = Table.read(f'{new_redshift_dir}/{root}-extracted.fits')['NUMBER']
    #results = vstack([
    #    Table.read(f"{new_redshift_dir}/{root}_{i:05d}.row.fits")
    #    for i in ids
    #    if os.path.exists(f"{new_redshift_dir}/{root}_{i:05d}.row.fits")
    #])
    #results.rename_column('root', 'field')
    #results.write(f"{new_redshift_dir}/{root}_fitresults_useEWprior.fits", overwrite=True)
